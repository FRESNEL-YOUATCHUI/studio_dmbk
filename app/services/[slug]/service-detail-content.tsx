...
export function ServiceDetailContent({ service }: { service: Service }) {
  ...
  return <>
    <section className="pt-32 px-6 bg-[#fff8ef] overflow-hidden">
      ...
      <motion.div style={{ y: heroY }} className="container mx-auto mt-14 relative h-[58vh] min-h-[420px] rounded-[36px] overflow-hidden border-[3px] border-black shadow-[12px_12px_0_#171717]"><Image src={service.heroImage} alt={service.title} fill priority sizes="100vw" className="object-cover scale-105"/></motion.div>
    </section>
    ...
    <section className="py-20 px-6 bg-[#171717] text-white"><div className="container mx-auto"><p className="eyebrow text-[#ffd447]">Ce qui est inclus</p>
      <div className="mt-12 space-y-8">{service.features.map((feature, i) => <motion.article key={feature.title} initial={{opacity:0,y:50}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:'-100px'}} className="grid md:grid-cols-2 gap-8 items-center border-t border-white/25 pt-8">
        ...
        <motion.div whileHover={{scale:1.02}} className={`relative aspect-[4/3] rounded-[28px] overflow-hidden ${i%2?'md:order-1':''}`}><Image src={feature.image} alt={feature.title} fill sizes="(max-width:768px) 100vw, 50vw" className="object-cover"/></motion.div>
    ...
    <section className="py-24 px-6 bg-[#ff8fab]"><div className="container mx-auto grid lg:grid-cols-2 gap-10"><div><p className="eyebrow">Ils en parlent</p><blockquote className="text-3xl md:text-5xl font-black leading-tight mt-6">“{service.testimonials[0]?.quote}”</blockquote><p className="mt-6 font-bold">{service.testimonials[0]?.author} — {service.testimonials[0]?.role}</p></div><div className="space-y-4">{service.faq.map(item=><details key={item.question} className="bg-white border-2 border-black rounded-2xl p-6 shadow-[5px_5px_0_#171717]"><summary className="font-black cursor-pointer">{item.question}</summary><p className="mt-4 leading-relaxed">{item.answer}</p></details>)}</div></div></section>

    <section className="py-24 px-6 bg-[#fff8ef] text-center"><h2 className="mega-title !text-[clamp(3rem,7vw,7rem)]">On crée quelque chose de <span>fort ?</span></h2><div className="flex flex-wrap justify-center gap-4 mt-10"><Link href={`/devis?service=${service.slug}`} className="creative-button">Demander un devis <ArrowRight/></Link><a href="https://wa.me/?text=Bonjour%20DMBK%20Studio" target="_blank" rel="noreferrer" className="creative-button bg-[#68d391]">WhatsApp <MessageCircle/></a></div></section>
